...
ENUM_TIMEFRAMES input TRADING_TIMEFRAME = PERIOD_M15;
...
double MA(int period, int shift, ENUM_TIMEFRAMES timeframe)
{
...
void ExecuteTrade()
{
   if (BarOpen())
   {
      ...
      // Calculate the 100-bar moving average
      double ma_100 = MA(100, 0, TRADING_TIMEFRAME);
      ...
      // Long order
      if (long_positions == 0 && ask > ma_100 && trigger_long - ask >= SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL))
      {
      ...
      // Short order
      if (short_positions == 0 && bid < ma_100 && bid - trigger_short >= SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL))
      {    
