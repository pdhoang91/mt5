...
bool BreakoutEntry(double &entry, double &tp, double &sl, bool &isBuy, 
                  ENUM_TIMEFRAMES timeframe, double VolumeThreshold, double BreakoutRiskRewardRatio)
{
    ...
    if(current_price > resistance && current_volume > VolumeThreshold * avg_volume)
    {
        ...
        // Calculate SL and TP
        sl = support;
        tp = entry + BreakoutRiskRewardRatio * (entry - sl);
        return true;
    }
    ...
    if(current_price < support && current_volume > VolumeThreshold * avg_volume)
    {
        ...
        // Calculate SL and TP
        sl = resistance;
        tp = entry - BreakoutRiskRewardRatio * (sl - entry);
        return true;
    }
